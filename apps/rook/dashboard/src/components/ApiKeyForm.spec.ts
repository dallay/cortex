    ...
    setup(props, {emit}) {
      return () =>
        h("input", {
          ...
          "data-testid":
            props.dataTestid ?? (props.id ? `input-${props.id}` : "input"),
          onInput: (e: Event) =>
  ...
  SelectTrigger: defineComponent({
    ...
    setup(props, {slots}) {
      return () =>
        h(
          "div",
          {id: props.id, "data-testid": props.dataTestid},
          slots.default?.(),
        );
    },
  }),
...
function makeProvider(
  ...
  return {
    ...
    config: {
      maxConcurrent: 1,
      quotaWindowThresholds: {warning: 0.5, error: 0.9},
      defaultModel: null,
      baseUrl: null,
    },
    testStatus: {
...
function makeFormState(
  overrides: Partial<ApiKeyFormState> = {},
): ApiKeyFormState {
  return {
    label: "",
...
describe("ApiKeyForm", () => {
  describe("scope rendering", () => {
    ...
    it("groups scopes under chat, providers, admin headings", () => {
      const wrapper = makeWrapper(makeFormState());
      expect(wrapper.find('[data-testid="scope-group-chat"]').exists()).toBe(
        true,
      );
      expect(
        wrapper.find('[data-testid="scope-group-providers"]').exists(),
      ).toBe(true);
      expect(wrapper.find('[data-testid="scope-group-admin"]').exists()).toBe(
        true,
      );
    });
    ...
    it("renders scope description text", () => {
      ...
      expect(wrapper.text()).toContain(
        "Read chat completions and conversation history",
      );
    ...
    it("flags the admin scope with a danger indicator", () => {
      const wrapper = makeWrapper(makeFormState());
      expect(wrapper.find('[data-testid="scope-danger-admin"]').exists()).toBe(
        true,
      );
    });
    ...
    it("does not flag non-admin scopes as dangerous", () => {
      ...
      for (const scope of SCOPES) {
        ...
        expect(
          wrapper
            .find(
              `[data-testid="scope-danger-${scope.value.replace(/[^a-zA-Z0-9_-]/g, "-")}"]`,
            )
            .exists(),
  ...
  describe("default scope checkboxes", () => {
    it("checks every scope by default EXCEPT admin", () => {
      ...
      for (const scope of SCOPES) {
        ...
        expect(
          checkbox.exists(),
          `checkbox for ${scope.value} should exist`,
        ).toBe(true);
        if (scope.value === "admin") {
          expect(
            checkbox.element.checked,
            "admin must be unchecked by default",
          ).toBe(false);
        } else {
          expect(
            checkbox.element.checked,
            `${scope.value} must be checked by default`,
          ).toBe(true);
        }
      }
    });
  ...
  describe("allowed providers", () => {
    ...
    it("checks the provider when toggled on", async () => {
      const wrapper = makeWrapper(makeFormState());
      const cb = wrapper.find<HTMLInputElement>(
        '[data-testid="provider-checkbox-p1"]',
      );
  ...
  describe("allowed models", () => {
    ...
    it("renders one checkbox per model, grouped by provider", () => {
      ...
      expect(wrapper.find('[data-testid="model-row-p1-gpt-4o"]').exists()).toBe(
        true,
      );
      expect(
        wrapper
          .find('[data-testid="model-row-p2-claude-3-5-sonnet-latest"]')
          .exists(),
      ).toBe(true);
    });
    ...
    it("checks the model when toggled on", async () => {
      const wrapper = makeWrapper(makeFormState());
      const cb = wrapper.find<HTMLInputElement>(
        '[data-testid="model-checkbox-p1-gpt-4o"]',
      );
  ...
  describe("tier", () => {
    ...
    it("renders all tier options in the dropdown", () => {
      ...
      for (const opt of TIER_OPTIONS) {
        ...
        expect(item.exists(), `tier option ${opt.value} should exist`).toBe(
          true,
        );
  ...
  describe("error display", () => {
    it("shows error when error prop is provided", () => {
      ...
      expect(wrapper.find('[data-testid="api-key-form-error"]').exists()).toBe(
        true,
      );
