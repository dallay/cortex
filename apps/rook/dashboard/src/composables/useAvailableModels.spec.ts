...
describe('useAvailableModels', () => {
  ...
  it('starts with empty state and no fetch', () => {
    ...
    return import('./useAvailableModels').then(async ({ useAvailableModels }) => {
  ...
  it('fetch() populates groups and sets fetched=true', async () => {
    mockGetAvailableModels.mockResolvedValue({
      models: [
        { providerId: 'p1', providerName: 'OpenAI Primary', providerKind: 'openai', models: ['gpt-4o'] },
        { providerId: 'p2', providerName: 'Anthropic', providerKind: 'anthropic', models: ['claude-3-5-sonnet-latest'] },
      ],
    })
    mockProviders.value = [
      { id: 'p1', name: 'OpenAI Primary', providerKind: 'openai', isActive: true },
      { id: 'p2', name: 'Anthropic', providerKind: 'anthropic', isActive: true },
    ]
  ...
  it('filters out inactive providers from the cross', async () => {
    mockGetAvailableModels.mockResolvedValue({
      models: [
        { providerId: 'p1', providerName: 'OpenAI Primary', providerKind: 'openai', models: ['gpt-4o'] },
        { providerId: 'p2', providerName: 'OpenAI Inactive', providerKind: 'openai', models: ['gpt-4o'] },
      ],
    })
    mockProviders.value = [
      { id: 'p1', name: 'OpenAI Primary', providerKind: 'openai', isActive: true },
      { id: 'p2', name: 'OpenAI Inactive', providerKind: 'openai', isActive: false },
    ]
  ...
  it('filters out groups with no models', async () => {
    mockGetAvailableModels.mockResolvedValue({
      models: [
        { providerId: 'p1', providerName: 'OpenAI', providerKind: 'openai', models: ['gpt-4o'] },
        { providerId: 'p2', providerName: 'Empty', providerKind: 'openai', models: [] },
      ],
    })
    mockProviders.value = [
  ...
  it('surfaces errors and keeps groups empty', async () => {
    ...
    mockProviders.value = [{ id: 'p1', name: 'OpenAI', providerKind: 'openai', isActive: true }]
