...
vi.mock('@/components/ui/button', () => ({
  Button: defineComponent({
    ...
    setup(props, { slots }) {
      return () => h('button', { type: 'submit', disabled: props.disabled }, slots.default?.())
    },
  }),
}))
vi.mock('@/components/ui/input', () => ({
  Input: defineComponent({
    props: { id: String, type: String, modelValue: String, class: String, autocomplete: String, required: Boolean },
...
describe('LoginView', () => {
  it('renders first-time admin setup when bootstrap is required', async () => {
    ...
    expect(wrapper.text()).toContain('You must set the admin password before using Rook')
    expect(wrapper.get('input#setup-password').attributes('type')).toBe('password')
  ...
  it('renders normal admin login when bootstrap is complete', async () => {
    ...
    expect(wrapper.get('input#login-password').attributes('type')).toBe('password')
  })
})
