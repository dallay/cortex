...
#### `[cache.signature_cache]` — Layer 1: Request Deduplication

Layer 1 cache uses SHA-256 signatures to deduplicate identical requests within the TTL window.
...
enabled = true
inspection_endpoints = true  # Enable GET /api/cache/signatures
```

| Field                  | Type | Default | Description                                              |
|------------------------|------|---------|----------------------------------------------------------|
| `enabled`              | bool | `true`  | Enable signature-based request deduplication              |
| `inspection_endpoints` | bool | `true`  | Enable signature inspection endpoints (`/api/cache/signatures`, `/api/cache/signature/:sig`) |

#### `[cache.token_cache]` — Layer 2: Provider-Side Token Caching

Layer 2 cache leverages provider-side caching (e.g., Anthropic's prompt caching) by injecting `cache-control: max-stale=3600` headers and parsing `x-cache` response headers to track hits.
...
| Field      | Type   | Default  | Description                                                           |
|------------|--------|----------|-----------------------------------------------------------------------|
| `mode`     | enum   | `"never"`| Cache mode: `auto` (known providers), `always` (all), `never` (off)   |
| `providers`| array  | `[]`     | Provider prefixes to enable. Empty uses built-in defaults              |
...
**Example: Enable token caching for Anthropic only:**

```toml
[cache.token_cache]
mode = "auto"  # or "always" for explicit
providers = ["anthropic"]
```

**Example: Enable token caching for multiple providers:**
...
1. Outbound requests to supported providers include `cache-control: max-stale=3600`
2. Providers return `x-cache: hit` for cached responses, `x-cache: miss` for new responses
3. Rook tracks hits/misses and calculates estimated cost savings
4. Metrics appear in `GET /api/cache/stats` under `token_cache` section
